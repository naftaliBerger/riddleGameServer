...
const server = http.createServer(async (req, res) => {
  if (req.method === "GET" && req.url === "/riddles") {
      ...
      const data = await fs.readFile("riddles.txt", "utf8");
      ...
      const newRiddle = await getRequestBody(req);

      const fileContent = await fs.readFile("riddles.txt", "utf8");
      const riddles = JSON.parse(fileContent);

      newRiddle.id = riddles.length + 1;  
      riddles.push(newRiddle);

      await fs.writeFile("riddles.txt", JSON.stringify(riddles));
